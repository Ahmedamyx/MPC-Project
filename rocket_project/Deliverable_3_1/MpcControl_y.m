classdef MpcControl_y < MpcControlBase
    ...
    methods
        ...
        function ctrl_opti = setup_controller(mpc, Ts, H)
            ...
            % Plot the maximum invariant set
            figure();
            plot(Xf.projection(1:2), 'g');
            title('X_f projected on dimension (1:2)');
            xlabel('$\omega_x$', 'interpreter', 'latex');
            ylabel('\alpha');
            figure();
            plot(Xf.projection(2:3), 'g');
            title('X_f projected on dimension (2:3)');
            xlabel('\alpha');
            ylabel('$v_y$', 'interpreter', 'latex');
            figure();
            plot(Xf.projection(3:4), 'g');
            title('X_f projected on dimension (3:4)');
            xlabel('$v_y$', 'interpreter', 'latex');
            ylabel('$y$', 'Interpreter', 'latex');

            % Desfine objectives and constraints
